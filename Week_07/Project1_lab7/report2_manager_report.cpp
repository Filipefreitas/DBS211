...
int main(void)
{
	...
	try
	{
		...
		ResultSet* rs = stmt->executeQuery("SELECT e.employeenumber, e.firstname, e.lastname, o.phone, e.extension FROM employees e LEFT JOIN offices o ON e.officecode = o.officecode WHERE e.reportsto = 1056 ORDER BY e.employeenumber");
